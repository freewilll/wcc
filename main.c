...
static char *make_default_output_filename(char *input, char *ext) {
    // Strip the path component so that the file path is gone.
    // The file will be saved in the current working directory.
    char *p = strrchr(input, '/');
    if (p) input = p + 1;

    // Replace or append the extension
    p  = strrchr(input, '.');
    if (!p) {
...
int main(int argc, char **argv) {
    ...
    while (argc > 0) {
        if (*argv[0] == '-') {
            ...
            else if (argc > 0 && !strcmp(argv[0], "--debug-function-return-value-mapping"   )) { debug_function_return_value_mapping = 1;    argc--; argv++; }
    ...
    if (help) {
        ...
        printf("--debug-function-return-value-mapping\n");
    ...
    get_debug_env_value("DEBUG_CALL_VALUE_ALLOCATION", &debug_call_value_allocation);
    ...
    get_debug_env_value("DEBUG_FUNCTION_RETURN_VALUE_MAPPING", &debug_function_return_value_mapping);
    ...
    for (int i = 0; i < input_filenames->length; i++) {
        ...
        char *compiler_output_filename =
            !run_assembler && !run_linker ? (output_filename ? wstrdup(output_filename) : make_default_output_filename(input_filename, "s"))
    ...
    if (run_assembler) {
        for (int i = 0; i < input_filenames->length; i++) {
            ...
            char *assembler_output_filename =
                !run_linker ? (output_filename ? wstrdup(output_filename) : make_default_output_filename(single_target ? input_filenames->elements[0] : input_filename, "o"))
